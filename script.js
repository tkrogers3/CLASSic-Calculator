...
class Model {
  //classes in Model -  storage array, num1, operator, num2, view, numDisplay. Anything else?

  constructor() {
    this.storeData = " ";  //storage string
    this.num1 = null;   //first number clicked 
    this.operator = null; //operator after first number clicked
    this.num2 = null;  //second number clicked
    this.view = null;  // view is set to null before model sends info to controller
    this.numDisplay = null; // Num display is set to null before Nums are sent to controller
...
class View {
  ...
  buildCalc() {
    ...
    let k = 0;
    let col;
    let btnChar = ["C", "", "", "/", "9", "8", "7", "X", "4", "5", "6", "-", "1", "2", "3", "+", "0", "", ".", "="];
    ...
    for (let i = 0; i < 5; i++) {
      ...
      for (let j = 0; j < 4; j++) {
        ...
        col.addEventListener("click", this.controller.handleKeyEvent.bind(this.controller));
...
//document.addEventListener("keydown", this.controller.handleKeyEvent.bind(this.c)); // sets up the binding of the keydown to the controller
//*///                                    c= new Controller 
v.buildCalc();
