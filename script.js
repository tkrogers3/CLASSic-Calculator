...
function buildElement(elementType, classes, id, textContent){
   ...
   function buildCalc() {
        ...
        let rowA = this.buildElement("div", "row border text-center bg-dark display-4 p-1", "output", "")
        display.appendChild(rowA)

        let colB = this.buildElement("div", "col-md-12 display-4 text-right p-1", "numberText", "3908")
        rowA.appendChild(colB)

        /*let rowB = this.buildElement("div", "row", "bg-black", "")
       display.appendChild(rowB) */
        ...
        for (let i = 0; i < 5; i++) {
            ...
            for (let j = 0; j < 4; j++) {
                col =this.buildElement("button", "col h3 m-2 border-1 p-2 text-secondary bg-dark", "btn" + k , btnChar[k] );
                k++
